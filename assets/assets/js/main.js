    ...
    if (elementName === "masterBoardSlide") {
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 32) { randomDraw(); } else if (e.keyCode === 82) { resetBoard(); } else if (e.keyCode === 88) { toggleBlocker(); } else if (e.keyCode === 66) { hideBingo('B', 'toggle'); } else if (e.keyCode === 73) { hideBingo('I', 'toggle'); } else if (e.keyCode === 78) { hideBingo('N', 'toggle'); } else if (e.keyCode === 71) { hideBingo('G', 'toggle'); } else if (e.keyCode === 79) { hideBingo('O', 'toggle'); } else if (e.keyCode === 84) { hide('masterBoardSlide');
                    show('settingsSlide', 'grid'); } else if (e.keyCode === 87) { hide('masterBoardSlide');
                    show('winningPatternSlide', 'grid'); } else if (e.keyCode === 86) { toggleBallsDrawnRemaining('toggle'); } else if (e.keyCode === 72) { hide('masterBoardSlide');
                    show('titleSlide'); } else if (e.keyCode === 70) { toggleFullScreen(); }
            }
        }
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 84 || e.keyCode === 13) { hide('settingsSlide');
                    show('masterBoardSlide', 'grid'); } else if (e.keyCode === 70) { toggleFullScreen(); }
            }
        }
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 87 || e.keyCode === 13) { hideBingoLettersBasedOnWinningPattern();
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 13) { hide('titleSlide');
                    show('masterBoardSlide', 'grid'); } else if (e.keyCode === 70) { toggleFullScreen(); }
            }
        }
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 37) { keyboardNavHowToUse(0); } else if (e.keyCode === 39) { keyboardNavHowToUse(1); } else if (e.keyCode === 70) { toggleFullScreen(); } else if (e.keyCode === 13) { hide('howToUseSlide');
                    show('titleSlide'); }
            }
        }
        ...
        document.onkeydown = function(e) {
            if (!keyPressed) {
                e.preventDefault();
                keyPressed = true;
                if (e.keyCode === 37) { keyboardNavCredits(0); } else if (e.keyCode === 39) { keyboardNavCredits(1); } else if (e.keyCode === 70) { toggleFullScreen(); } else if (e.keyCode === 13) { hide('aboutCreditsSlide');
                    show('titleSlide'); }
            }
        }
    } else {
...
function changeBG(color) {
  let newColor;
  const classesToUpdate = ["body", "bingoLetter", "controlsCol", "bingoCol", "bingoBallVintage","bingoBallVintageActive"]; // Corrected typo
  const idsToUpdate = ["drawBallButton"];

  if (color === "dark") {
    newColor = "#000000";
    document.getElementById("blocker").style.backgroundImage = "linear-gradient(#000000, #000000)";
  } else if (color === "blue") {
    newColor = "#051C37";
    document.getElementById("blocker").style.backgroundImage = "linear-gradient(#051C37, #051C37)";
  } else if (color === "light") {
    newColor = "#F2F4F6";
    document.getElementById("blocker").style.backgroundImage = "linear-gradient(#F2F4F6, #FFFFFF)";
  } else {
    newColor = "#051C37";
    document.getElementById("blocker").style.backgroundImage = "linear-gradient(#051C37, #051C37)";
  }

  // Target classes and IDs together
  const elements = [
    ...document.querySelectorAll(classesToUpdate.map(cls => `.${cls}`).join(',')),
    ...idsToUpdate.map(id => document.getElementById(id))
  ];
  elements.forEach((element) => {
    if (element) {
      element.style.color = color === "dark" || color === "blue" ? "#FFFFFF" : "#051C37";
      element.style.borderColor = color === "dark" || color === "blue" ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.1)";
    }
  });
...
function easeFontSize(element, startSize, endSize, duration) {
    const steps = 40; // Number of easing steps
...
function activateBingoBall(bingoIDNum) {
    ...
    if (saveData.drawnBingoBalls.indexOf(bingoIDNum) === -1) {
        ...
        document.getElementById("bigBingoBall").classList.remove(
            document.getElementById("bigBingoBall").classList.item(1)
        );
        ...
        const bigBingoNumber = document.getElementById("bigBingoNumber");

        // Smooth easing of fontSize
        easeFontSize(bigBingoNumber, 120, 95, 2000);
        ...
        saveData.drawnBingoBalls.splice(
            saveData.drawnBingoBalls.indexOf(bingoIDNum),
            1
        );
...
function toggleBallsDrawnRemaining(renderOrToggle) {
    if (renderOrToggle === "toggle") {
        ...
        if (saveData.ballsDrawnRemaining === "hidden") {} else if (saveData.ballsDrawnRemaining === "drawn") {
            document.getElementById("ballsDrawnRemaining").style.visibility = "visible";
            document.getElementById("ballsDrawn").style.display = "flex";
        } else {
            document.getElementById("ballsDrawnRemaining").style.visibility = "visible";
...
function setUpSettings() {
    ...
    if (saveData.themeColor === "dark") {
        document.getElementById("dark").style.backgroundColor = "#B6C9D7";
    } else if (saveData.themeColor === "blue") {
        document.getElementById("blue").style.backgroundColor = "#B6C9D7";
    } else if (saveData.themeColor === "light") {
        document.getElementById("light").style.backgroundColor = "#B6C9D7";
    }
    if (saveData.bingoStyle === "ball") {
        document.getElementById("bingoStyleBall").style.backgroundColor = "rgba(255,255,255,0.3)";
    } else if (saveData.bingoStyle === "vintage") {
        document.getElementById("bingoStyleVintage").style.backgroundColor = "rgba(255,255,255,0.3)";
    }
}
